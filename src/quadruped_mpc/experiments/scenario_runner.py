...
from quadruped_mpc.plants import srb as SRB_model
from quadruped_mpc.core.rotations import rpy_to_matrix
from quadruped_mpc.control import convex_mpc as cvx_mpc
from quadruped_mpc.control.gait_planning import get_gait_parameters, get_contact_state
...
def run_scenario(
    ...
    for sim_cnt in range(clock.n_steps):
        ...
        #   지령 위치 스텝간 점프       17.0 mm   <- 한 물리 스텝(0.11ms) 안에!
        #
        # 비교 기준을 정확히 해 둔다. 지령 p_feet_wf 는 물리 스텝(9kHz)마다가
        # 아니라 **스윙 틱(1kHz)마다만** 바뀐다. 그래서 물리 격자에서 재는
        # 스텝간 최대 점프의 정상값은 '스윙 속도 x 물리 dt'(0.16mm)가 아니라
        # '스윙 속도 x 스윙 dt'다 — trot 1 m/s 에서 최대 스윙 속도가 약
        # 3 m/s 이므로 3 mm 근처가 정상이다(실측 3.02 mm).
        # 결함 13 의 서명은 이 값이 그 기준을 몇 배로 넘는 것이었다(17/3 ≈ 5.7).
